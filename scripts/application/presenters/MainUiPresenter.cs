	...
	private bool TryHandleDeveloperModeInput(string text)
	{
		...
		if (command.Equals("/dev activate", StringComparison.OrdinalIgnoreCase))
		{
			...
			_chatPanelView.AddMessage(
				new ChatMessageVm(
					"DEV",
					"Developer mode activated. Enter REST commands like GET /state, /auto pass, /next, /random simulation, or /test path random simulation. Use /dev deactivate to exit."
				)
			);
			return true;
		}
